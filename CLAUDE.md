...
- **可変次数 N-gram**。n=1..max_n(既定 3)の全ての遷移を同時に学習し、
  生成時は各ステップで n を一様乱数で引いて、直前 n トークンを context key に
  次トークンを確率サンプリングする。選んだ n の context が未知なら小さい n に
  back off。これで n を固定する場合の「小さい n だとバラバラ / 大きい n だと
  ほぼ元文そのまま」を中和する。文末記号で文を区切る。
